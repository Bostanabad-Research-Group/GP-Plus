...
Runs default MatrixEncoder first, then NeuralEncoder.
Uses grouped (flat) categorical one-hot columns.
"""
...
for seed in range(start_seed, start_seed + num_seeds):
    ...
    for encoder_mode in encoder_modes:
        ...
        if encoder_mode == "matrix":
            ...
            print("---------------------------------------------------")
            print("Using MatrixEncoder for cat encoder")
            print("---------------------------------------------------")
            cat_encoder = gpplus.utils.MatrixEncoder(
                input_dim=len(grouped_cat),
            ...
            print("---------------------------------------------------")
            print("Using NeuralEncoder for cat encoder")
            print("---------------------------------------------------")
        ...
        model = GPR(X_train.clone(), y_train.clone(), kernel_module=kernel)
        print(model)
        ...
        print(f"Seed {seed} metrics ({encoder_mode}):")
...
for encoder_mode, run_metrics in metrics_by_encoder.items():
    ...
    print(f"\n=== Buckling SF (grouped flat cat, {run_label}Encoder) ===")
    print(
