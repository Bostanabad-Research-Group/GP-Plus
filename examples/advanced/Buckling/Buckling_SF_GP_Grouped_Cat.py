"""
Single-fidelity Buckling GP: one source (s0) only.

Matches experiments_revisions_april/A2_buckling_SF_GPvsPFN.py GP path:
  - float64 tensors (A2 uses gp_dtype=torch.float64; data_gen here is float32 by default)
  - LogScaleKernel(MVMFKernel(...)) with cat_cols = flattened blocks like A2's grouped_cat
  - LBFGSScipy + same-style optimizer kwargs
  - cholesky_jitter on GPTrainer

The grouped failure you saw was float32 Cholesky hitting a stiff K_train; A2 never used float32.
"""
...
for seed in range(start_seed, start_seed + num_seeds):
    ...
    cont_cols = [9]
    # Same E/K/I block layout as separate script; A2 builds grouped_cat by flattening blocks:
    grouped_cat = [0, 1, 2, 3, 4, 5, 6, 7, 8]
    ...
    # Critical: match A2 — GP in float64 (load_data_buckling_MF is float32)
...
print("\n=== Buckling SF (grouped flat cat like A2) ===")
print(
...
# Grouped OH 2+4+3 (E,K,I). Without qual_dict, latent reps assume 9 levels → 9 rows.
